...
steve_edwards = Customer("Steve Edwards", "4017 Ringdove Way, Roanoke, Tx", "dryer", "Maytag", "no heat", None, None,
                         None, None)
cx_lst.append(cx_1)
...
if len(tech_lst) == 1:
    ...
    for cx in cx_lst:
        if int(str(get_dist(tech_lst[0].home, cx.address)).split(" ")[0]) < int(
                str(get_dist(tech_lst[1].home, cx.address)).split(" ")[0]):
            tech_lst[0].cx_lst.append(cx.cx_address())
        else:
...
def get_lat_long(tech_route):
    ...
    df = pd.DataFrame(
        {"address": tech_route, "latitude": loc_lat, "longitude": loc_long, "formatted_address": formatted_addys})
    waypoints = []
